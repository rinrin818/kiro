...
- **분야별 도구는 해당 분야 카테고리**로 분류한다.
  Figma·Sketch·Adobe = "디자인"  /  VSCode·GitHub·프레임워크 = "개발"  /  Jira·Linear = "기획·PM".
  "도구·서비스"는 분야 횡단 범용 도구(예: Notion, Slack, 캘린더, 메모앱)에만 사용한다.
- 카테고리가 둘 사이에서 모호하면 **콘텐츠가 다루는 분야(주제)**를 기준으로 선택한다.
  예: "디자이너용 새 Figma 기능 출시" → "디자인" (도구가 아니라 디자인 분야 소식).
